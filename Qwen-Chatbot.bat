REM .\Qwen-Chatbot.bat
@echo off
setlocal enabledelayedexpansion

REM ==== Static Configuration ====
set "TITLE=Qwen-Chatbot"
title %TITLE%
...
REM ============================================================
REM  Detect Python 3.12 from common locations
REM ============================================================
:DetectPython
set "PYTHON_EXE="
set "BEST_MAJOR=0"
...
if not defined PYTHON_EXE (
    echo Error: No compatible Python 3.12 found.
...
for /f "tokens=2 delims= " %%V in ('"%pyexe%" --version 2^>^&1') do (
    for /f "tokens=1,2 delims=." %%A in ("%%V") do (
        set "MAJ=%%A"
        set "MIN=%%B"
        if !MAJ!==3 if !MIN! GEQ 11 if !MIN! LEQ 12 (
