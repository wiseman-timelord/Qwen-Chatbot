...
### Development
- STT - We could have a STT button in the tools section, enabling the input box to switch to a sample display and a button, the user would click and hold the button to record, and then let go of mouse when they finished recording, and then the wave appear in the box, then AI translate this into words, then when finished translating wave turn into these words, as editable text in input box, and the wave record box will hide, but there will be a new button at bottom of text input when STT is enabled, to switch back to the STT Recording box and hide the text box again, so the user can re-record (blanking the previous recorded text upon pressing record). if the user selects STT again to disable it then, the wave box will hide, the text input box will be shown, and the Re-Record button will be hidden. Hmm. Is this the best way to do this? needs a brainstorm, but we want to have annotated conversations with AI in teh conversation log, and ability to edit the given annotations and resend, so one would assume that cancelling here would then make the previous stt annotated recording active text in the editing box, while presented as if the user has not commited to saying that line yet. This would require button for tool on right of User Input box, that then switch the User Input box for 2/3 of vertical from top as a live wave display to show sound input through mic, while underneath a single line of annotation, with dynamic expanding slider bar under. Research...
```
| Library | Strengths | Weaknesses | Best For |
| :--- | :--- | :--- | :--- |
| **`faster-whisper`** | **Fastest CPU inference**, low memory, supports quantization, high accuracy, Windows compatible. | Requires downloading model files (~75MB for `small.en`). | **Real-time transcription**, voice assistants, applications needing low latency. |
| **`Vosk`** | **Very lightweight** (~50MB models), works offline, supports many platforms (including Raspberry Pi). | Slightly lower accuracy than Whisper, fewer advanced features. | Embedded systems, mobile apps, projects where model size is the absolute priority. |
| **`RealtimeSTT`** | Built-in voice activity detection (VAD) and wake word support. | Higher-level abstraction may add overhead; less focused on raw efficiency. | Prototypes and applications that need integrated audio processing features. |
| **`SpeechRecognition`** | Wrapper for multiple engines (Google, Sphinx, etc.), easy to use. | Requires internet for many engines; not fully offline. | Projects that don't require offline operation or have minimal accuracy needs. |
```

### Reasoning
- There is no Think/NoThink button, because most quantized models are NOT intended to run relevantly in modes they are not designed for, as the idea is GGUF are cut down in order to fit in some small/snappy file format. Additionally configuring 2 models would introduce complexity, it would be better to make the same program but for other libraries such as OV, that possibly have Think/NoThink buit-in, but this is not intended as a "Agentic Framework", its more a good/sinmple Chatbot using one-mode-does-all kinds of models.
